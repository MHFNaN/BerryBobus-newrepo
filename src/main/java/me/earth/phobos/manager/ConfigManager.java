...
public class ConfigManager
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("skobos").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("skobos/" + name + "/")) ? "skobos/" + name + "/" : "skobos/config/";
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("skobos/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("skobos", ""));
                writer.close();
            } else {
